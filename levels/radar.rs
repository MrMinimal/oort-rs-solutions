...
impl Ship {
    ...
    pub fn tick(&mut self) {
        if let Some(contact) = scan() {
            ...
            let intercept_dir = Self::calculate_intercept_dir(false, contact.position, contact.velocity);
    ...
    pub fn calculate_intercept_dir(debug: bool, target_pos: Vec2, target_velocity: Vec2) -> Vec2 {
        let vec_to_target = target_pos + position();
